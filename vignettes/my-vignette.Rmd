...
---Answer: please see below comparation results when using Rcpp and C++
...
```{r}
system.time({brute_force_knapsack(x = knapsack_objects[1:500,], W = 3500)})
system.time({brute_force_knapsack_parallel(x = knapsack_objects[1:500,], W = 3500)})
```
